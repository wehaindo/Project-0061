  ...
  const PaymentPrima = PaymentInterface.extend({
    ...
    _prima_check: async function () {
        ...
        if (self.latest_prima_qris_status_count > 4){
          ...
          if(confirmed){
            await this._request_prima_qris_status(order, paymentLine);        
          }else{
            await this._request_latest_prima_qris_status(order, paymentLine);    
          }              
        }else{
          await this._request_latest_prima_qris_status(order, paymentLine);  
        }                       
    ...
    _request_prima_qris: async function (order, paymentLine) {        
        var self = this;
