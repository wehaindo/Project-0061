...
class ProductPriceChange(models.Model):
    ...
    def action_view_product(self):
        return {
            ...
            'view_mode': 'tree',
            'views': [
                (self.env.ref('weha_smart_pos_aeon_price.product_price_change_line_view_tree').id, 'tree'),                
                ],
    ...
    def action_process(self):        
        for line in self.line_ids:
            vals = {
                ...
                'member_fixed_price': line.member_fixed_price,
    ...
    def action_download_product(self):
        pass
    ...
    products_file = fields.Binary('Products File')
    products_filename = fields.Char('Products Filename')
...
class ProductPriceChangeLine(models.Model):
    ...
    price_change_id = fields.Many2one('product.price.change', 'Price Change#', ondelete='cascade', index=True)
    product_template_id = fields.Many2one('product.template', 'Product', index=True)
    product_product_id = fields.Many2one('product.product', 'Variants', index=True)
