...
class PricelistItem(models.Model):
    ...
    def remove_product_pricelist_item(self):
        product_pricelist_item_id = self.with_context({'is_delete': True})
        auth = HTTPBasicAuth('admin','pelang1')
        headers = {
            'Content-Type': 'application/json'
        }        
        ...
        if product_pricelist_item_id.product_pricelist_item_couchdb_id:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if response.status_code == 200 or response.status_code == 201:                        
                        _logger.info("Pricelist Item Deleted")        
                        # vals = {
                        #     'product_pricelist_item_couchdb_sync': False,
                        #     'product_pricelist_item_json': "",
                        #     'product_pricelist_item_couchdb_id': "",
                        #     'product_pricelist_item_couchdb_rev': ""
                        # }                
    ...
    def cleaning_product_pricelist_item(self, retain_days=-7):    
        current_date_time = datetime.now()
        retain_date_time = current_date_time + timedelta(days=retain_days)
        domain  = [
            ('date_end','<=', retain_date_time),
            ('product_pricelist_item_couchdb_sync','=', True)
        ]
        ...
        for product_pricelist_item_id in product_pricelist_item_ids:
            _logger.info("Delete Pricelist Item")
    ...
    def write(self, vals):        
        ...
        is_delete = self.env.context.get('is_delete')
        if not is_delete:
            self.action_sync()
        return res
    ...
    def unlink(self):
        for row in self:
            row.remove_product_pricelist_item()
        super(PricelistItem, self).unlink()    
