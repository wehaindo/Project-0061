odoo.define("weha_smart_pos_aeon_pos_data.db", function(require) {
    ...
    async function loadUserConfig()
    {
        let promise = new Promise((resolve, reject) => {
            ...
            db.query(mapUserTable).then(function (result) {
                resolve(result);
            }).catch(function (err) {
                reject(err);
            });
    ...
    DB.prototype.get_product_by_barcode = async function(barcode){
        console.log('get_product_by_barcode');
        var self = this;
        var product = get_product_by_barcode_prototype.apply(this, arguments);
        if(product){
            return product;
        }
        
        let promise = new Promise((resolve, reject) => {                    
            self.product_barcode_pouch_db_conn.createIndex({
                    index: {fields:['barcode']}                    
            }).then(function() {
                self.product_barcode_pouch_db_conn.find({
                    selector: {
                        'barcode': barcode
                    }
                }).then((products) => {
                    ...
                    console.log(products);
                    var docs_product = products['docs'][0];
                    var product = self.product_by_id[docs_product.id];                    
                    console.log('Product');
                    console.log(product);
                    resolve(product);
                }).catch(function(err){
    ...
    // DB.prototype.get_product_by_barcode = async function(barcode){
    //     console.log('get_product_by_barcode');
    //     // get_product_by_barcode_prototype.apply()
    //     var self = this;
    //     var result_product = undefined;
    //     if (this.set_is_product_barcode_pouchdb){            
    //         console.log("PouchDB Product Barcode");            
    //         await this.product_barcode_pouch_db_conn.createIndex({
    //             index: {fields:['barcode']}                    
    //         }).then(async function() {
    //             await self.product_barcode_pouch_db_conn.find({
    //                 selector: {
    //                     'barcode': barcode
    //                 }
    //             }).then((products) => {
