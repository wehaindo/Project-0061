// let access_token =  "access_token_273da7f4966e11b7cb313c6c64c1cb64584d0540";
let cart = [];
let keyInput = '';
let index = 0;
let allUsers = [];
...
let allTransactions = [];
let cartList = [];
let productLastUpdate = Date.now();
let sold = [];
let state = [];
let sold_items = [];
let item;
...
var dbOrder = new PouchDB('order');
var dbHold = new PouchDB('hold');
...
$(document).keypress(function(event) {
    if(event.charCode === 13){
        ...
        keyInput = keyInput + String.fromCharCode(event.which);
        console.log('Handler for .keypress() called. - ' + event.charCode);
        console.log('Handler for .keypress() called. - ' + String.fromCharCode(event.which));
    }
});

$(document).ready(function () {
    $(".loading").hide();        
    $('#loggedin-user').text(res_users.name);
    ...
    function loadLocalProducts() {
        dbInventory.allDocs({
            ...
            rows.forEach(row => {
                item = row.doc;
                dbInventory.put(item);
                dbInventory.put(item, function(err, response) {
                ...
                let item_info = `
                    <div class="col-lg-2 box ${item.category._id}" onclick="addToCart('${item._id}', ${item.quantity}, ${item.stock})">
                        <div class="widget-panel widget-style-2 ">                    
                            <div id="image">
                                <img src="/weha_smart_pos/static/pos/images/default.jpg" id="product_img" alt="">
                            </div>                    
                            <div class="text-muted m-t-5 text-center">
                                <div class="name" id="product_name">${item.product_name}</div> 
                                <span class="sku">${item.sku}</span>
                                <span class="stock">STOCK </span><span class="count">${item.stock}</span>
                            </div>
                            <sp class="text-success text-center"><b data-plugin="counterup">${item.price}</b> </sp>
                        </div>
                    </div>`;
            ...
            categories.forEach(category => {
                $('#categories').append(`<button type="button" id="${category._id}" class="btn btn-categories btn-white waves-effect waves-light">${category.name}</button> `);
            });
        }).catch(function(err) {
            console.log(err);
        });
    ...
    function loadProducts() {
        ...
        var settings = {
            "url": "/api/smartpos/v1.0/products",
            "method": "GET",
            "headers": {
    ...
    function loadOrders(){
        ...
        if(isEmpty(pos_order)){
            $('#pos-order-number').text("Order # : No Order");
        }else{
            $('#pos-order-number').text("Order # : " + pos_order.name);
            console.log("Load Order Lines");
            var settings = {
                "url": "/api/smartpos/v1.0/orderlinebyorder/" + pos_order._id,
                "method": "GET",
                "timeout": 0,
                "headers": {
                  "access-token": access_token,
    ...
    function loadPaymentMethods(){
        console.log("Load Payment Methods");
        
        var settings = {
            "url": "/api/smartpos/v1.0/paymentmethods",
            "method": "GET",
            "headers": {
                "access-token": access_token,
            },
        ...
        .done(function(data) {
            ...
            data.forEach(item => {
                if (first){
                    first = false;
                    paymentmethod = `<a href="javascript:void(0)" id="${item._id}" onclick="paymentType = '${item._id}'" class="list-group-item active">${item.payment_method_name}</a>    `
                }else{
                    paymentmethod = `<a href="javascript:void(0)" id="${item._id}" onclick="paymentType = '${item._id}'" class="list-group-item">${item.payment_method_name}</a>    `
                }
                $('#payment-method-list').append(paymentmethod);
            });        
        });
    }
    ...
    function barcodeSearch(e) {
        ...
        $.ajax({
            url: '/api/smartpos/v1.0/product/' + $("#skuCode").val() ,
            type: 'GET',
    ...
    $.fn.deleteFromCart = function (index) {
        ...
        var settings = {
            "url": "/api/smartpos/v1.0/deleteposorderline",
            "method": "POST",
            "timeout": 0,
            "headers": {
              "access-token": access_token,
    ...
    $.fn.submitDueOrder = function (status) {
        ...
        cartList.forEach(item => {
            items += "<tr><td>" + item.product_name + "</td><td>" + item.quantity + "</td><td>" + parseFloat(item.price).toFixed(2) + "</td></tr>";
        });
        ...
        var paymenType = "d4a3849e-de68-48fd-a39a-8c5b0f429119";
        ...
        var settings = {
            "url": "/api/smartpos/v1.0/processpayment",
            "method": "POST",
            "timeout": 0,
            "headers": {
              "Content-Type": "application/json",
            ...
            "data": JSON.stringify({
              "smart_pos_order_couch_id": pos_order._id,
              "smart_pos_payment_method_couch_id": paymenType,
              "amount_paid": paid,
          ...
          .done(function (response) {
            ...
            <table width="100%">
                ...
                <tr>
                    <td>Discount</td>
                    <td>:</td>
                    <td>${discount > 0 ? parseFloat(discount).toFixed(2) : ''}</td>
                </tr>
                ...
                <tr>
                    ...
                    <td><h3>:</h3></td>
                    <td>
                        <h3>${parseFloat(orderTotal).toFixed(2)}</h3>
                    </td>
                </tr>
                ${payment == 0 ? '' : payment}
                </tbody>
    ...
    $.fn.getHoldOrders = function () {
        var settings = {
            "url": "/api/smartpos/v1.0/holds/" + pos_config._id + "/" + res_users._id,
            "method": "GET",
            "timeout": 0,
            "headers": {
              "access-token": access_token,
                    ...
                    $('<a>').append(
                        $('<div>', {
                            ...
                            $('<button>', {
                                ...
                                onclick: '$(this).orderDetails(' + index + ')'
                            }).append(
                                $('<span>', {
    ...
    $.fn.orderDetails = function (index) {
        if (pos_order) { 
            holdPosOrder();
        }
        pos_order = holdOrderList[index];
        loadOrders();
    ...
    $('#settings_form').on('submit', function (e) {
        ...
        if (formData.percentage != "" && !$.isNumeric(formData.percentage)) {
            ...
            storage.setItem('settings', formData);
            location.reload();
...
function addToCart(id, count, stock) {
    dbInventory.get(id, function(err, doc) {
        if (err) { return console.log(err); }
        console.log('AddProductToCart');
        console.log(doc);
        addProductToCart(doc);
    })

    // var settings = {
    //     "url": "/api/smartpos/v1.0/product/" + id,
    //     "method": "GET",
    //     "timeout": 0,
    //     "headers": {
    //         "access-token": access_token,
...
function addProductToCart(product) {
    ...
    if (isEmpty(pos_order)) {
        //Create POS Order
        console.log("Create Pos Order");
        console.log('Pos Order Empty');
        data_order = {
            "user_couch_id": res_users._id,
            "smart_pos_config_couch_id": pos_config._id,
        }
        var settings = {
            "url": "/api/smartpos/v1.0/createposorder",
            "method": "POST",
            "timeout": 0,
            "headers": {
              "Content-Type": "application/json",
        ...
        .done(function (response) {
            console.log(response);
            pos_order = response.result.data;
            if(pos_order.err){
                swal.fire(
                    'Error create pos order', 
                    'Error : ' + pos_order.message,
                    'warning');
            }else{
                $('#pos-order-number').text("Order # : " + pos_order.name);
                product.quantity = 1;
                createPosOrderLine(product);
            }
...
function keySearch(){
    ...
    if(keyInput){
        dbInventory.get()
        // let product = allProducts.find(function (selected) {
        //     return selected.barcode === keyInput;
        // });
        let product = dbInventory.find({
            selector: {
                name: {$eq: keyInput}
            }
        })
        if(product){
            ...
            Swal.fire(
                'Not Found!',
                '<b>' + keyInput + '</b> is not a valid barcode!',
                'warning'
            );
            keyInput = '';
        }
...
function createPosOrderLine(data){
    console.log('Pos Order Not Empty');
    data_order_line = {
        smart_pos_order_couch_id : pos_order._id,
        description: data.product_name,
        product_couch_id: data._id,
        quantity: data.quantity,
        price_unit: data.price,
        amount_total: data.price * data.quantity,
        amount_tax: 0,
        amount_discount: 0
    }
    var settings = {
        "url": "/api/smartpos/v1.0/createposorderline",
        "method": "POST",
        "timeout": 0,
        "headers": {
          "Content-Type": "application/json",
        ...
        "data": JSON.stringify(data_order_line),
    };
    
    $.ajax(settings)
    .done(function (response) {
        console.log(response);
        cartList.push(response.result.data);
        renderTable();     
    })
...
function addProductToCartLine(index, data){
    console.log('addProductToCartLine');
    console.log(data);
    $('#cartTable > tbody').append(
        $('<tr>').append(
            ...
            $('<td>', {
                text: "Rp" + (data.price * data.quantity)
            }),
            $('<td>').append(
                $('<button>', {
...
function holdPosOrder(){
    if (cartList.length != 0) {
        Swal.fire({
            title: 'Are you sure to hold this order?',
            text: "Order will move to hold orders",
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
            confirmButtonText: 'Yes, Hold this order!'
        }).then((result) => {
            if (result.value) {
                var settings = {
                    "url": "/api/smartpos/v1.0/processhold",
                    "method": "POST",
                    "timeout": 0,
                    "headers": {
                      "Content-Type": "application/json",
...
function renderTable() {
    ...
    $.each(cartList, function (index, data) {
        addProductToCartLine(index, data);
    });
    
    $('#cartTable > tbody').stop(true, false).animate({
        scrollTop: $('#cartTable > tbody').get(0).scrollHeight}, 2000);
};

function calculateCart() {
    let total = 0;
    let grossTotal;
    $('#total').text(cartList.length);
    $.each(cartList, function (index, data) {
        total += data.quantity * data.price;
    });
    total = total - $("#inputDiscount").val();
    $('#price').text("Rp" + total.toFixed(2));
    ...
    $("#gross_price").text("Rp" + grossTotal.toFixed(2));
    ...
    $('#loading').append(
        `<div id="load">
            <form id="account">
                ...
                <div class="form-group">
                    <input type="text" placeholder="Pos Config" name="pos_config" class="form-control">
                </div>
                <div class="form-group">
...
$('body').on("submit", "#account", function (e) {
    ...
    if (formData.username == "" || formData.password == "") {
        ...
        dbUser.get('1').then((doc) => {
            storage.setItem('auth', {
                auth: true
            });
            storage.setItem('user', doc);
            location.reload();
        }).catch((err) => {
            console.log(err);
            Swal.fire(
