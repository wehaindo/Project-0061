...
platform = storage.getItem('settings');
...
dbUser.get("1").then((doc) => {
    $('#loggedin-user').text(doc.fullname);
}).catch((err) =>{
    console.log(err);
});
...
$(document).ready(function () {
    ...
    function loadProducts() {
        ...
        var settings = {
            "url": "/api/smartpos/v1.0/products/all",
            "method": "GET",
            "timeout": 0,
            "headers": {
        ...
        $.ajax(settings).done(function(data) {
            ...
            data.forEach(item => {
                ...
                let item_info = `<div class="col-lg-2 box ${item.category}" onclick="$(this).addToCart('${item._id}', ${item.quantity}, ${item.stock})">
    ...
    function loadCategories() {
        $.get('/api/categories/all', function (data) {
            ...
            allCategories.forEach(category => {
                ...
                $('#category').append(`<option value="${category.doc._id}">${category.doc.name}</option>`);
            });
        });
    }
    ...
    //function addToCart(id, count, stock){
        if (stock == 1) {
            if (count > 0) {
                var settings = {
                    "url": "/api/smartpos/v1.0/products/" + id,
                    "method": "GET",
                    "timeout": 0,
                    "headers": {
                        "access-token": access_token,
    ...
    $.fn.addProductToCart = function (data) {
        ...
        // db = window.indexedDB.open("smartposDB", 1);
        var cart = db.transaction("carts","readwrite");
        var cartStore = cart.objectStore("carts");
        ...
        var request = cartStore.add(item,1);
