...
class price_change_file(models.Model):
    ...
    def check_product(self, sku):
        ...
        if res:
            ...
            mes = True
            res = self.env['product.product'].search([(('product_tmpl_id','=', res.id))],limit=1)
            return mes, res
        else:
    ...
    def check_product_pricelist_item(self, pricelist_id, product_id, price_change_code=False):
        ...
        if not price_change_code:
            res = self.env['product.pricelist.item'].search([('pricelist_id','=', pricelist_id.id),('product_tmpl_id','=',product_id.id)], limit=1)
        else:
            res = self.env['product.pricelist.item'].search([('pricelist_id','=', pricelist_id.id),('product_tmpl_id','=',product_id.id),('prc_no','=', price_change_code)], limit=1)
    ...
    def create_product_pricelist_item(self, pricelist_id, product_id, rows):
        ...
        vals = {
            ...
            'product_tmpl_id':  product_id.id, #Product Template
            'product_id':  product_product_id.id #Product ID
        }
...
class PriceChangeLine(models.Model):
    ...
    def _process_download(self):
        ...
        _logger.info(active_id)
        with api.Environment.manage(), self.pool.cursor() as new_cr:
            self = self.with_env(self.env(cr=new_cr)) -> set new cursor jadi nggak pakai cursor odoo ini bebas cpu time limit bisa jalan berhari hari           
            self.env['price.change.file']._import_data(active_id) -> function untuk import data

   def process_download(self):     
   	threaded_download = threading.Thread(target=self._process_download, args=()) -> panggil function _process_download
       	threaded_download.start()                       
        return {'type': 'ir.actions.client', 'tag': 'reload'}                 
