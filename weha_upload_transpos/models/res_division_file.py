...
class res_division_file(models.Model):
    ...
    def generate_file(self, id):
        ...
        try:
            ...
            for line in file_lines:
                ...
                if rows['rec_id'] == "D":
                    ...
                    #division = self.env['res.division'].search([('code','=', rows['div'])], limit=1)
                    #division.archive()
            
            res_division_file_id.date_end = datetime.now()
            res_division_file_id.state = 'done'
            self.env.cr.commit()

        except Exception as e:
            _logger.error(e)
            res_division_file_id.date_end = datetime.now()
            res_division_file_id.state = 'error'
            self.env.cr.commit()
        ...
        # except Exception as e:
        #     _logger.error(e)
        #     self.date_end = self.env['res.division.file'].browse(id).date_end = datetime.now()
        #     self.state = 'error'
        #     # self.env.cr.commit()
    ...
    state = fields.Selection([('draft','New'),('in_progress','In Progress'),('done','Finished'),('cancel','Cancel'),('error',"Error")], 'Status', default='draft')
