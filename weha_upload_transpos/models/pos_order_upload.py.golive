...
class pos_order_file(models.Model):
    ...
    def generate_upload_file(self):
        store_codes = ["7001",'7002','7003']
        file_paths = ["/profit/idn/upload/pos/7001/","/profit/idn/upload/pos/7002/","/profit/idn/upload/pos/7003/"]
        for i in range(2):            
            vals = {
                'date': datetime.now()
            }
            res = self.env['pos.order.file'].create(vals)
            res.get_pos_order_upload(store_codes[i])
            # res.action_save_to_directory(store_codes[i])

    def action_save_to_directory(self, store_code):
        # /comm/idn/upload/pos/7001/
        # /profit/idn/upload/pos/7001/
        self.ensure_one()
        _logger.info('Save File to Comm')
        try:
            with open("/profit/idn/upload/pos/" + store_code + "/" + self.filename.upper(),  "wb") as fh:            
    ...
    def item_record(self, line, row, is_duplicate):
        ...
        if row.product_id.default_code:                        
            sku = str(row.product_id.default_code).zfill(13)
        else:
            product_template_id = row.product_id.product_tmpl_id  
            sku = str(product_template_id.default_code).zfill(13)
        ...
        if line.is_void == False and line.is_refund == False:
            ...
            actual_sell = f"{calc_actual_sell:0.2f}".zfill(23)
    ...
    def total_record(self, line, row, is_duplicate):
        ...
        if line.aeon_member:
            member_card = line.aeon_member[:15].ljust(20, " ")
        else:
    ...
    def payment_record(self, line, row, payment, change_amount, is_duplicate):
        ...
        payment_amount = f"{(payment.amount + change_amount):0.2f}".zfill(23)
        # payment_amount = f"{(payment.amount):0.2f}".zfill(23)
        ...
        actual_amt = f"{(payment.amount + change_amount):0.2f}".zfill(23)
    ...
    @api.model
    def get_pos_order_upload(self, store_code, start_date=False, end_date=False):
        # Define variable
        s_file_type_a = []

        if not start_date:
            start_date_time = datetime.now() # + timedelta(days=-1)
            str_start_date = str(start_date_time.year)+ "-" + str(start_date_time.month).zfill(2) + "-" + str(start_date_time.day).zfill(2) + " " + "00:00:00"
        ...
        if not end_date:
            end_date_time = datetime.now() # + timedelta(days=-1)
            str_end_date = str(end_date_time.year) + "-" + str(end_date_time.month).zfill(2) + "-" + str(end_date_time.day).zfill(2) + " " + "23:59:59"
        ...
        sqlgroup="""
            ...
            WHERE b.code = '{}' AND a.create_date BETWEEN '{}' AND '{}'
            GROUP BY a.branch_id, b.code         
        """.format(store_code, start_date_time, end_date_time) # .format(start_date_time, end_date_time) WHERE create_date BETWEEN '{}' AND '{}'        
        ...
        if len(groups) > 0:
            for group in groups:
                ...
                for order in orders:
                    ...
                    group_results  = self.env['pos.payment'].read_group(
                        [('pos_order_id','=', order.id)],
                        ['pan:min','merchant_id:min','approval_code:min','terminal_id:min','amount:sum'],
                        ['payment_method_id']
                    )                                      
                    payment_ids = []
                    for group_result in group_results:
                        payment_id = self.env['pos.payment'].new()
                        payment_id.payment_method_id = self.env['pos.payment.method'].browse(group_result['payment_method_id'][0])
                        payment_id.pan = group_result['pan']
                        payment_id.merchant_id = group_result['merchant_id']
                        payment_id.approval_code = group_result['approval_code']
                        payment_id.terminal_id = group_result['terminal_id']
                        payment_id.amount = group_result['amount']       
                        payment_ids.append(payment_id)
                    _logger.info(payment_ids)
                    
                    for payment in payment_ids:
                        if payment.payment_method_id.code == "CSH":   
                            payment_record = self.payment_record(order, row, payment, 0, is_duplicate) # TYPE C
                            total_payment = total_payment + payment.amount
                            output.write(payment_record)
                            if order.is_void == False and order.is_refunded == False:
                                _logger.info('Calculate Rounding For Normal Transaction') 
                                if order.amount_paid < order.amount_total:
                                    _logger.info('Rounding')
                                    payment_record = self.payment_rounding(order, row, is_duplicate)
                                    total_cash_amount = total_cash_amount - (order.amount_total - order.amount_paid)
                                    output.write(payment_record)
                                    # _logger.info(payment_record)
                            else:                                
                                _logger.info('Calculate Rounding For Void Or Refund Transaction')
                                if order.amount_paid > order.amount_total:
                                    _logger.info('Rounding')
                                    payment_record = self.payment_rounding(order, row, is_duplicate)
                                    total_cash_amount = total_cash_amount - (order.amount_paid - order.amount_total)
                                    output.write(payment_record)
                        else:                                
                            payment_record = self.payment_record(order, row, payment, 0, is_duplicate) # TYPE C
                            total_payment = total_payment + payment.amount
                    ...
                    # for payment in payment_ids:                        
                    #     if payment.payment_method_id.code == "CSH":        
                    #         cash_lines = payment_ids.filtered(lambda r:r.payment)                                                                 
                    #         if not payment.is_change:
                    #             _logger.info('CSH')
                    #             change_amount = 0                            
                    #             cash_lines = []    
                    #             for cash_payment in order.payment_ids:
                    #                 if cash_payment.is_change:
                    #                     change_amount = change_amount + cash_payment.amount
                    #                 else:
                    #                     if cash_payment.payment_method_id.code == "CSH":
                    #                         cash_lines.append(cash_payment)
                    #             payment_line = cash_lines[0]                                           
                    #             total_cash_amount = total_cash_amount + (payment_line.amount + change_amount)                 
                    #             _logger.info('CSH Detail')
                    #             _logger.info(str(payment_line.amount) + '|' + str(change_amount))
                    #             payment_record = self.payment_record(order, row, payment_line, change_amount, is_duplicate)
                    ...
                    if order.is_refunded == False and order.is_void == False:
                        sales_rcpt_c += 1
                    else:
                        return_rcpt_c += 1
                ...
                _logger.info('total_payment')
                _logger.info(total_payment)
                _logger.info('total_cash_amount')
                _logger.info(total_cash_amount)
                _logger.info('total_cash')
                _logger.info(total_cash)
    ...
    def create(self, vals):
        ...
        res.action_save_to_directory(vals['flag_number'])
