...
class PosConfig(http.Controller):    
    ...
    @validate_token
    @http.route('/pos/decentralize/config/get', type="http", auth="none", methods=["POST"], csrf=False)
    def config_get(self, **kwargs):     
        ...
        fields = ['id','name','currency_id','receipt_header',
                  ...
                  'available_pricelist_ids','company_id','payment_method_ids','res_branch_id','branch_code']
    ...
    @validate_token
    @http.route('/pos/decentralize/config/open', type="http", auth="none", methods=["POST"], csrf=False)
    def config_open(self, **kwargs):     
        try:
            ...
            if pos_config_id:
                if not pos_config_id.has_active_session:
