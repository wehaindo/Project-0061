...
class PosDownload(http.Controller):    
    ...
    def convert_to_utc(self, local_time, system_tz):
        """
        Convert a given time to UTC based on the system's timezone.

        :param local_time: Datetime object in local timezone
        :param system_tz: String representing the system's timezone
        :return: Datetime object in UTC
        """
        if not local_time or not system_tz:
            return None

        # Load the system timezone
        system_timezone = timezone(system_tz)
        
        # Localize the time (attach the system timezone to the naive datetime)
        localized_time = system_timezone.localize(local_time, is_dst=None)
    ...
    def download_items(self, **kwargs): 
        ...
        local_start_dt = datetime.strptime(date_request + ' 00:00:00', '%Y-%m-%d %H:%M:%S')
        local_start_tz = pytz.timezone('Asia/Jakarta')
        localized_start_time = local_start_tz.localize(local_start_dt)
        local_end_dt = datetime.strptime(date_request + ' 23:59:00', '%Y-%m-%d %H:%M:%S')
        local_end_tz = pytz.timezone('Asia/Jakarta')
        localized_end_time = local_end_tz.localize(local_end_dt)
        
        utc_start_time = localized_start_time.astimezone(pytz.utc)     
        utc_end_time = localized_end_time.astimezone(pytz.utc)
        
        domain = [
            ('branch_id','=', branch_id.id),
            ('data_type','=', data_type),
            ('name','>=',utc_start_time.strftime('%Y-%m-%d %H:%M:%S')),
            ('name','<=',utc_end_time.strftime('%Y-%m-%d %H:%M:%S'))
        ]
