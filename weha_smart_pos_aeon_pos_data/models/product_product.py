...
class ProductTemplate(models.Model):
    ...
    def _prepare_product_json(self, branch_id):
        _logger.info("Prepare Product Json 1")
        def json_serial(obj):
            """JSON serializer for objects not serializable by default json code"""
            if isinstance(obj, (datetime, date)):
                return obj.isoformat()
    ...
    def generate_product_json(self):
        ...
        for branch_id in self.branch_ids:
            product_template_couchdb_id, product_couchdb_id, product = self._prepare_product_json(branch_id.id)
            products_json = json.dumps(product, default=json_serial)
    ...
    def generate_product_json_by_branch(self, branch_id):
        ...
        if branch_id:
            _logger.info("_prepare_product_json_by_branch")
    ...
    def sync_product_by_branch(self, branch): 
        ...
        #     product_template_data_ids = self.product_template_data_ids
        # Rubah
        domain = [('branch_id', '=', branch.id),('product_template_id','=',self.id)]
    ...
    def remove_product(self, branch):
        ...
        if product_template_data_id.product_couchdb_id:
            ...
            _logger.info(product_template_data_id.product_couchdb_id)
            try:
                response = requests.get(f'{branch_id.couchdb_server_url}/{branch_id.couchdb_name}/{product_template_data_id.product_couchdb_id}', verify=False)
                _logger.info(response.status_code)
                if response.status_code == 200:
                    _logger.info("Product Found")
                    json_data = response.json()
