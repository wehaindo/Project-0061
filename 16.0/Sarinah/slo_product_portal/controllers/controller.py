...
class CustomerPortal(portal.CustomerPortal):
    ...
    def portal_my_product(self, **kwargs):
        products = request.env['product.template'].sudo().search([])
        return request.render("slo_product_portal.show_product", {'products': products, 'page_name': 'product'})

    @http.route('/my/products/<model("product.template"):product>/', auth='user', website=True)
