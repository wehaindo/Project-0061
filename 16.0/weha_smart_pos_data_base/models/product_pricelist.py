...
class PricelistItem(models.Model):
    ...
    def _prepare_json(self):
        ...
        product_pricelist_items = self.env['product.pricelist.item'].search_read(domain=[('id','=', self.id)], fields=self.env['pos.session']._product_pricelist_item_fields())        
        ...
        try:
            if self.product_pricelist_item_couchdb_id:
                ...
                if response.status_code == 200:
                    ...
                    response = requests.put(f'{branch_id.couchdb_server_url}/{branch_id.couchdb_product_pricelist_items_db}/{couchdb_product_pricelist_item["_id"]}', auth=auth, headers=headers, data=product_pricelist_item_json, verify=False)
                    if response.status_code == 200 or response.status_code == 201:
                        _logger.info("Success")
                elif response.status_code == 404:
                    response = requests.post(f'{branch_id.couchdb_server_url}/{branch_id.couchdb_product_pricelist_items_db}', auth=auth, headers=headers, data=self.product_pricelist_item_json, verify=False)
