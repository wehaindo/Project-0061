...
class PosHome(http.Controller):    
    ...
    @http.route('/pos/finger/<code>', type='http', auth="none")
    def pos_finger(self, code, redirect=None, **kw):
        request.session.logout()
        values = {}
        if 'message' in request.params:
            message = request.params.get('message')
        ...
        if pos_config_id:
            ...
            values['pos_session_id'] = False
        #values['pos_session_id'] = pos_session_id
        response = request.render('weha_smart_pos_login.pos_finger', values)
        return response
    ...
    def pos_scan(self, **post):
        ...
        try:
            ...
            if not pos_config:
                return request.redirect('/pos/finger/' + pos_config_id.pos_config_code + '?message=POS Config not found')
            ...
            if pos_config.current_session_id:
                if pos_config.current_session_id.user_id.id != uid:
                    return request.redirect('/pos/finger/' + pos_config.pos_config_code + '?message=POS already used by ' + pos_config.pos_session_username  + '!')                        
            ...
            if uid:
                ...
                return request.redirect('/pos/finger/' + pos_config.pos_config_code + '?message=Username or Password failed')                    
