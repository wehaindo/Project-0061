...
class Agreement(models.Model):
    ...
    branch_ids = fields.Many2many('res.branch', string='Store')
    ...
    top_id = fields.Many2one(
        string='Term of Payment',
        comodel_name='account.payment.term',
        ondelete='restrict',
    )
    rebate_product_ids = fields.One2many(
        comodel_name="product.template",
        string="Products",
        inverse_name="agreement_id",
    )
    
    rebate_product_product_ids = fields.One2many(
        string='Products',
    ...
    @api.depends(
        "rebate_product_product_ids"
    )
    def _compute_rebate_domain(self):
        for line in self:
            rebate_domain = []
            rebate_domain = [("product_id", "in", line.rebate_product_product_ids.ids)] or []
