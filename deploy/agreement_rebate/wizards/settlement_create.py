...
class AgreementSettlementCreateWiz(models.TransientModel):
    ...
    def _get_target_model_pos(self):
        return self.env["report.pos.order"]
    ...
    def _prepare_target_domain_pos(self):
        domain = [
            ("state", "not in", ["draft", "cancel", "paid", "invoiced"]),
        ]
        if self.journal_ids:
            domain.extend([("journal_id", "in", self.journal_ids.ids)])
        else:
    ...
    def _target_line_domain(self, agreement_domain, agreement, line=False):
        ...
        # if line:
        domain += safe_eval(agreement.rebate_domain)
    ...
    def _target_line_domain_pos(self, agreement_domain, agreement, line=False):
        domain = agreement_domain.copy()
        if agreement.start_date:
            domain.append(
                (
        ...
        # if line.rebate_product_product_ids:
        domain += safe_eval(agreement.rebate_domain)
        # elif agreement.rebate_product_product_ids:
        #     domain = expression.AND(
        #         [
        #             domain,
        #             expression.OR(
        #                 [safe_eval(x.rebate_domain) for x in agreement.rebate_product_product_ids]
        #             ),
        #         ]
        #     )
        return domain
    ...
    def get_agregate_fields_pos(self):
        return [
            "price_total",
        ]

    def _get_amount_field_pos(self):
        return "price_total"

    def _prepare_settlement_line(
        self, domain, group, agreement, line=False, section=False
    ):
        if self.domain == "pointofsale":
            amount = group[self._get_amount_field_pos()] or 0.0
        else:
        ...
        vals_product = []
        vals_id = []
        domain_product += safe_eval(agreement.rebate_domain)
        move_line_obj = self.env["account.move.line"].search([('move_id','=', group["move_id"][0]),("product_id","in", agreement.rebate_product_product_ids.ids)])

        for move_line in move_line_obj:
            vals_product.append((6,0, move_line))
            vals_id.append((6,0, move_line))

        vals.update(
        {
            "move_id": group["move_id"][0],
            "invoice_line_ids": move_line,
            # "rebate_product_ids": vals_product,
        })

        _logger.info('......PRODUCT')
        _logger.info(domain_product)
        _logger.info(move_line_obj)
        _logger.info(vals_product)
        # _logger.info(move_line.line_ids.ids)
        return vals
    ...
    def action_create_settlement(self):
        ...
        if self.domain == "pointofsale":
            target_model = self._get_target_model_pos()
        else:
            target_model = self._get_target_model()
        if self.domain == "pointofsale":
            orig_domain = self._prepare_target_domain_pos()
        else:
        ...
        for agreement in agreements:
            ...
            if agreement.rebate_type == "line":
                ...
                for line in agreement.rebate_line_ids:
                    if self.domain == "pointofsale":
                        domain = self._target_line_domain_pos(
                            agreement_domain, agreement, line=line
                        )
                        groups = target_model.sudo().read_group(
                            domain,
                            self.get_agregate_fields_pos(),
                ...
                if self.domain == "pointofsale":
                    domain = self._target_line_domain_pos(agreement_domain, agreement)
                    groups = target_model.sudo().read_group(
                        domain,
                        self.get_agregate_fields_pos(),
                        self._settlement_line_break_fields(),
                ...
                if (
                    ...
                    continue
                if self.domain == "pointofsale":
                    amount = groups and groups[0][self._get_amount_field_pos()] or 0.0
                else:
                ...
                if self.domain == "pointofsale":
                    ...
                    #     lazy=False,
                    # )
                    # coba = [('state', 'not in', ['draft', 'cancel']), ('journal_id', 'in', [2]), ('invoice_date', '>=', '2023-03-01'), ('invoice_date', '<=', '2023-03-31'), ('invoice_date', '>=', '2023-03-13'), ('invoice_date', '<=', '2023-03-31')]
                    groups = target_model.read_group(
                        domain=domain,
                        fields=['move_id','price_subtotal'],
                        groupby=['partner_id','move_id'],
                        lazy=False,
                    )
                    _logger.info('.....global-groups')
                    _logger.info(groups)
                if (
                    not groups
                    or not groups[0]["__count"]
