...
class SaleOrder(models.Model):
    ...
    bu_id = fields.Many2one('business.unit', string='Business Unit', store=True, compute="_compute_bu")
    branch_id = fields.Many2one("res.branch", string='Operating Unit', store=True,
                                readonly=False,
    ...
    def _compute_bu(self):
        for order in self:
            order.bu_id = False
            if len(self.env.user.bu_ids) == 1:
                company = self.env.company
                so_company = order.company_id if order.company_id else self.env.company
                bu_ids = self.env.user.bu_ids
                bu = bu_ids.filtered(
                    lambda bu: bu.company_id == so_company)
                if bu:
                    order.bu_id = bu.ids[0]
                else:
                    order.bu_id = False
...
class SaleOrderLine(models.Model):
    ...
    bu_id = fields.Many2one('business.unit', string='Business Unit', store=True,)
