...
class PosLoyalty(models.Model):
    ...
    point_per_order = fields.Float(string='Point Per Order')
    point_per_product = fields.Float(string='Point Per Product')
    loyalty_rule = fields.One2many('pos.loyalty.rule', 'loyalty_id', string='Rules')
    loyalty_reward = fields.One2many('pos.loyalty.reward', 'loyalty_reward_id', string='Reward')
...
class PosLoyaltyRule(models.Model):
    ...
    point_per_product = fields.Float(string='Point Per Product')
    loyalty_id = fields.Many2one('pos.loyalty')
...
class PosLoyaltyReward(models.Model):
    ...
    loyalty_reward_id = fields.Many2one('sh.pos.loyalty')
    reward_type = fields.Selection([('gift', 'Gift'), ('discount','Discount')], default='gift', required=1)
    product_ids = fields.Many2one('product.product', string='Gift Product')
    point_cost = fields.Float(string='Point Cost')
    minimum_point = fields.Float(string='Minimum Points')
