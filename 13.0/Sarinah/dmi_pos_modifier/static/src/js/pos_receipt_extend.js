odoo.define("sh_pos_receipt_extend.pos", function (require) {
    ...
    screens.ReceiptScreenWidget.include({
        show: async function () {
            ...
            // Menandai status barcode dan QR sebagai exit
            order["is_barcode_exit"] = true;
            order["is_qr_exit"] = true;

            if (this.pos.config.sh_pos_order_number) {
                setTimeout(async function () {
                    try {
                        // Mengambil data pos.order berdasarkan pos_reference
                        var callback = await rpc.query({
                            model: "pos.order",
                            method: "search_read",
                        ...
                        if (callback && callback[0] && callback[0]['name']) {
                            // Menyimpan nomor pesanan ke order
                            order["pos_order_number"] = callback[0]['name'];

                            // Mengambil data untuk rendering receipt
                            var datas = await self.get_receipt_render_env();

                            // Mengupdate tampilan receipt
                            self.$(".pos-receipt-container").html(
                                qweb.render("OrderReceipt", datas)
                            );

                            // Menangani auto-print jika diatur
        ...
        render_receipt: function () {
            ...
            $.ajax({
                ...
                error: async function () {
                    ...
                    self.$(".pos-receipt-container").html(qweb.render("OrderReceipt", await self.get_receipt_render_env()));
                },
