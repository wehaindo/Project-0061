...
class report_account_general_ledger(models.AbstractModel):
    ...
    def _get_options_domain(self, options):
        ...
        if options.get('department') and options.get('department_ids'):
            domain += [
                ('department_id', 'in', options.get('department_ids') )
            ]
    ...
    def _get_aml_line(self, options, account, aml, cumulated_balance):
        ...
        department_id = aml_id.department_id.name or ''
        label = aml_id.name or ''
        reference = aml_id.move_id.ref or ''
        tax_number = aml_id.move_id.l10n_id_tax_number or ''
        ...
        return {
            ...
            'columns': [
                ...
                {'name': label, 'class': 'whitespace_print'},
                {'name': tax_number, 'class': 'whitespace_print'},
                {'name': reference, 'class': 'whitespace_print'},
                ...
                {'name': department_id, 'class': 'whitespace_print'},
