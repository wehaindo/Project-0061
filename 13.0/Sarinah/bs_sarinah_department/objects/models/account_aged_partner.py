...
class AccountAgedPartner(models.AbstractModel):
    ...
    def _get_columns_name(self, options):
        columns = [
            ...
            {'name': _("Invoice/Bill Date"), 'class': 'date', 'style': 'white-space:nowrap;'},
            {'name': _("Document Date"), 'class': 'date', 'style': 'white-space:nowrap;'},
            ...
            {'name': _("Department"), 'class': '', 'style': 'text-align:center; white-space:nowrap;'},
            ...
            if 'partner_%s' % (values['partner_id'],) in options.get('unfolded_lines'):
                for line in amls[values['partner_id']]:
                    ...
                    vals = {
                        ...
                        'columns': [{'name': v} for v in [format_date(self.env, aml.date_maturity or aml.date), aml.move_id.invoice_date, aml.move_id.document_date,aml.journal_id.code, aml.account_id.display_name, aml.department_id.name or '', aml.branch_id.name or '', format_date(self.env, aml.expected_pay_date)]] +
...
class ReportAgedPartnerBalance(models.AbstractModel):
    ...
    def _get_partner_move_lines(self, account_type, date_from, target_move, period_length):
        ...
        if ctx.get('department_ids'):
            department_clause = 'AND (l.department_id IN %s)'
            arg_list += (tuple(ctx['department_ids'].ids),)
        ...
        query = '''
            ...
            WHERE (l.account_id = account_account.id)
                ...
                ''' + branch_clause + department_clause +'''
        ...
        if ctx.get('department_ids'):
            department_ids = [department.id for department in ctx['department_ids']]
        else:
            department_ids = []
        ...
        for i in range(5):
            ...
            if branch_ids:
                ...
                args_list.append(tuple(branch_ids))
                args_list = tuple(args_list)
            if department_ids:
                args_list = list(args_list)
                args_list.append(tuple(department_ids))
                args_list = tuple(args_list)
            query_select = """SELECT l.id
                    ...
                    AND l.company_id IN %s"""
            if branch_ids:
                query_select += ' AND (l.branch_id in %s) '
            if department_ids:
                query_select += ' AND (l.department_id in %s) '
            query = query_select + ' ORDER BY COALESCE(l.date_maturity, l.date)'
        ...
        query_select = """SELECT l.id
                ...
                AND (l.date <= %s)
                AND l.company_id IN %s"""
        query_param = (tuple(move_state), tuple(account_type), date_from, tuple(partner_ids), date_from, tuple(company_ids))
        if branch_ids:
            query_select += ' AND (l.branch_id in %s) '
            query_param = list(query_param)
            query_param.append(tuple(branch_ids))
            query_param = tuple(query_param)
        if branch_ids:
            query_select += ' AND (l.department_id in %s) '
            query_param = list(query_param)
            query_param.append(tuple(department_ids))
            query_param = tuple(query_param)
        query = query_select + ' ORDER BY COALESCE(l.date_maturity, l.date)'
