...
class AccountReport(models.AbstractModel):
    ...
    def _init_filter_branch(self, options, previous_options=None):
        ...
        selected_department_ids = [int(dep) for dep in options['department_ids']]
        selected_departments = selected_department_ids and self.env['hr.department'].browse(selected_department_ids) or self.env['hr.department']
    ...
    def _set_context(self, options):
        ...
        if options.get('department_ids'):
            ctx['department_ids'] = self.env['hr.department'].browse([int(department) for department in options['department_ids']])
    ...
    def get_report_informations(self, options):
        ...
        if options.get('department'):
            options['selected_department_ids'] = [self.env['hr.department'].browse(int(department)).name for department in options['department_ids']]
