...
class AccountCashFlowReport(models.AbstractModel):
    ...
    def _get_liquidity_move_report_lines(self, options, currency_table_query, payment_move_ids, payment_account_ids):
        ...
        if options.get('department_ids'):
            ...
            if len(department_list) == 1:
                department = department_list[0]
                account_credit_query = """ AND credit_line.department_id = %s""" % (str(department))
                account_debit_query = """ AND debit_line.department_id = %s""" % (str(department))
            else:
                departments = tuple(list(set(department_list)))
                account_credit_query = """ AND credit_line.department_id in %s""" % (str(tuple(departments)))
                account_debit_query = """ AND debit_line.department_id in %s""" % (str(tuple(departments)))
        ...
        if options.get('department_ids'):
            ...
            if len(department_list) == 1:
                department = department_list[0]
                account_query = """ AND line.department_id = %s""" % (str(department))
            else:
                departments = tuple(list(set(department_list)))
                account_query = """ AND line.department_id in %s""" % (str(tuple(departments)))
