...
var ReceiptScreenWidget = ScreenWidget.extend({
    ...
    print_web: async function() {
        if ($.browser.safari) {
            document.execCommand('print', false, null);
        } else {
            try {
                setTimeout(async () => {
                // Membuat Promise yang akan menyelesaikan setelah pencetakan selesai
                const printAndWait = new Promise((resolve, reject) => {
                    window.onafterprint = () => resolve();  // Menyelesaikan setelah print selesai
                    window.onerror = (err) => reject(err);  // Menangani error jika ada masalah saat print
                });

                // Memanggil print
                window.print();

                // Tunggu sampai pencetakan selesai
                await printAndWait;

                // Melakukan aksi setelah pencetakan selesai
    ...
    print_html: async function () {
        var datas = await this.get_receipt_render_env();
        var receipt = QWeb.render('OrderReceipt', datas);
console.log('receipt');
    ...
    htmlToImg: function (receipt) {
        var self = this;
        if (this.pos.config.is_mobile_pos){
            $('.pos-receipt-print').css({'font-size': '28' });
        }
...
//       this.htmlToImg(receipt).then(value => console.log(value));
        ...
        if(typeof Android !== "undefined" && Android !== null) {
            ...
            this.htmlToImg(receipt).then(value => Android.printImg(value, is_mobile));
//            this.htmlToImg(receipt).then(value => console.log(value));
//            Android.printImg(receipt);
        }
        else{
            var datas = await this.get_receipt_render_env();
            var receipt = QWeb.render('OrderReceipt', datas);
        ...
        // !Custom By LLHa
            await this.print_web();

        } else {    // proxy (html) printing
            await this.print_html();
            this.lock_screen(false);
        }
    },
    click_next: function() {
    ...
    render_receipt: async function() {
        var datas = await this.get_receipt_render_env();
        this.$('.pos-receipt-container').html(QWeb.render('OrderReceipt', datas));
        console.log('receipt');
    },
});
