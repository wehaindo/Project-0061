                        ...
                        if (promotion_list && promotion_list[0]) {
                            _.each(promotion_list, function (promotion) {
                                if ((Number(promotion.from_time) <= current_time && Number(promotion.to_time) > current_time) || (!promotion.from_time && !promotion.to_time)) {
                                    ...
                                    else if (promotion && promotion.promotion_type == "percent_discount_quantity_item") {
                                        // Check if current product is in the promotion's product list
                                        if (promotion.product_amt_multi_ids && promotion.product_amt_multi_ids.includes(line.product.id)) {
                                            var line_ids = [];

                                            var matching_orderlines = _.filter(order.get_orderlines(), obj => promotion.product_amt_multi_ids.includes(obj.product.id));
                                            var total_qty = _.reduce(matching_orderlines, (sum, orderline) => sum + orderline.get_quantity(), 0);
                                            var max_discount = 0;
...
//                                                var line_record = _.find(pos_qty_discount_amt, function (obj) { return obj.id == pos_quntity_amt_id });
//                                                if (line_record) {
//                                                    if (total_qty >= line_record.quantity_amt) {
//                                                        if (line_record.discount_percent_price) {
//                                                            var discount_before_quantity = line_record.discount_percent_price;
//                                                        }
//                                                    }
//                                                }
//                                            });
                                            ...
                                            _.each(promotion.pos_quntity_amt_ids || [], function(pos_quntity_amt_id) {
                                                var line_record = _.find(pos_qty_discount_amt || [], function(obj) {
                                                    return obj.id == pos_quntity_amt_id;
                                                });

                                                if (line_record && line_record.quantity_amt <= total_qty) {
                                                    if (line_record.discount_percent_price > max_discount) {
                                                        max_discount = line_record.discount_percent_price;
                                                    }
                                                }
                                            });
...
//                                            }
                                            ...
                                            if (max_discount > 0) {
                                                _.each(matching_orderlines, function(promo_line) {
                                                    if (!promo_line) return;

                                                    promo_line.set_discount(max_discount);
                                                    promo_line.set_quantity_discount({
                                                        'rule_name': promotion.promotion_code,
                                                    });
                                                    promo_line.set_is_rule_applied(true);
                                                    ...
                                                    // MCS Promotion Data
                                                    promo_line.custom_promotion_id = promotion?.id;
                                                    promo_line.vendor_id = promotion?.vendor_id?.[0] ?? null;
                                                    promo_line.vendor_shared = promotion?.vendor_shared ?? false;
                                                    promo_line.sarinah_shared = promotion?.sarinah_shared ?? false;
                                                });
