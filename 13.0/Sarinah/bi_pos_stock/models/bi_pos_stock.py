...
class pos_config(models.Model):
    ...
    pos_hide_product = fields.Boolean(string="Hide Product not available Stock in POS")
    pos_stock_type = fields.Selection(
        [
            ("onhand", "Qty on Hand"),
            ("incoming", "Incoming Qty"),
            ("outgoing", "Outgoing Qty"),
            ("available", "Qty Available"),
        ],
        string="Stock Type",
        help="Seller can display Different stock type in POS.",
    )
    pos_allow_order = fields.Boolean(
        string="Allow POS Order When Product is Out of Stock"
    )
    pos_deny_order = fields.Char(
        string="Deny POS Order When Product Qty is goes down to"
    )
...
class pos_order(models.Model):
    ...
    def create_picking(self):
        ...
        _logger.debug("======== 1st : %s", datetime.now() - dt)
        for order in self:
            ...
            _logger.debug("======== 1st - inside 1st for -  before 2nd for : %s", datetime.now() - dt,)
                ...
                moves |= Move.create(
                    {
                        ...
                        if line.qty >= 0
                        else return_picking.id,
                        "picking_type_id": picking_type.id
                        if line.qty >= 0
                        else return_pick_type.id,
            ...
            _logger.debug("======== 1st - inside 1st for -  after 2nd for : %s",datetime.now() - dt,)

            # prefer associating the regular order picking, not the return
            order.write({"picking_id": order_picking.id or return_picking.id})

            _logger.debug("======== 1st - inside 1st for -  after 2nd for after write : %s",datetime.now() - dt,)

            if return_picking:
                _logger.debug("======== return picking")
                #order._force_picking_done(return_picking)  # lemoooot
                cr = self.env.cr
                cr.execute("SELECT yyt_fn_assign_picking(%s, %s); SELECT yyt_fn_validate_picking(%s, %s);",(return_picking.id, self.env.user.id,return_picking.id, self.env.user.id),)
            if order_picking:
                #order._force_picking_done(order_picking)  # lemoooot
                _logger.debug("======== order picking")
                cr = self.env.cr
                cr.execute("SELECT yyt_fn_assign_picking(%s, %s); SELECT yyt_fn_validate_picking(%s, %s);",(order_picking.id, self.env.user.id,order_picking.id, self.env.user.id),)

            _logger.debug("======== 1st - inside 1st for -  after 2nd for after force_picking : %s", datetime.now() - dt,)
            ...
            _logger.debug("======== 1st - inside 1st for -  after 2nd for after move : %s",  datetime.now() - dt, )

        _logger.debug("======== 2nd : %s", datetime.now() - dt)
        return True
...
class stock_quant(models.Model):
    ...
    def get_single_product(self, product, location):
        ...
        quants = self.env["stock.quant"].search(
            [("product_id", "=", pro.id), ("location_id", "=", location["id"])]
        )
        if len(quants) > 1:
            quantity = 0.0
            for quant in quants:
            ...
            if len(quants) > 1:
                ...
                if not quants:
                    ...
                    product.available_quantity = (
                        quants.quantity - product_qty + incoming_qty
                    )
                    res.update(
                        {product.id: quants.quantity - product_qty + incoming_qty}
                    )
        return [res]
