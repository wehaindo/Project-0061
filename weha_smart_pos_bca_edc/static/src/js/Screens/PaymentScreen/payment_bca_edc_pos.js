odoo.define('weha_smart_pos_bca_edc.payment', function (require) {
    'use strict'
    ...
    const { ConnectWS } = require('weha_smart_pos_bca_edc.WebSocketClient')
    ...
    const PaymentBcaEdc = PaymentInterface.extend({
      // connection: null,
      // connection_status: false,
        
      // start_ws_client: function () {
      ...
      // send_ws_client_message: function (data) {
      ...
      send_payment_request: function () {
        ...
        this._reset_state()
        return this._bca_edc_pay()
      },
  
      get_selected_payment: function () {
        const paymentLine = this.pos.get_order().selected_paymentline
        if (paymentLine && paymentLine.payment_method.use_payment_terminal === 'bca_edc') {
          return paymentLine
        }
        return false
      },
      ...
      _bca_edc_pay: async function () {
        console.log("BCA EDC PAY")
        const self = this
        var webSocket = new ConnectWS();        
        webSocket.start()
        const order = this.pos.get_order()
        const paymentLine = this.get_selected_payment()
        if (paymentLine && paymentLine.amount <= 0) {
          this._show_error(
            _t('Cannot process transaction with zero or negative amount 1.')
          )
          return Promise.resolve()
        }
        ...
        await webSocket.sendMessage({"message": "message sending"})
