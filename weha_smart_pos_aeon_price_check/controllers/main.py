        ...
        if store_id:                    
            ...
            if product_id:
                ...
                strSQL = """SELECT a.promotion_code, 
                    ...
                    LEFT JOIN pos_promotion_res_branch_rel f on f.promotion_id = a.id
                    WHERE  a.from_date <= '{}' AND a.to_date >= '{}' 
                    AND a.promotion_type = 'buy_x_partial_quantity_get_special_price' 
                    AND e.id={}
                    AND f.branch_id={}""".format(datetime.now().strftime('%Y-%m-%d'), datetime.now().strftime('%Y-%m-%d'), product_id.id, store_id.id)
                _logger.info(strSQL)                
