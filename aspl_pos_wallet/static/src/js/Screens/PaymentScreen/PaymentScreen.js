odoo.define('aspl_pos_wallet.PaymentScreen', function(require) {
    ...
    const WalletFrPaymentScreen = PaymentScreen =>
        class extends PaymentScreen {
            constructor(){
                super(...arguments);
                this.state = useState({ remaining_wallet_amount: false});
            }
            get_client_remaining_amount(){
                return this.env.pos.get_client() ? this.env.pos.get_client().remaining_wallet_amount : null;
            }
            deletePaymentLine(event){
                ...
                if(line.payment_method.allow_for_wallet){
                    this.currentOrder.set_to_wallet(!this.currentOrder.is_to_wallet());
                    this.state.remaining_wallet_amount += line.amount;
                }
                NumberBuffer.reset();
                this.render();
            }
            async createPaymentLine(){
                ...
                for ( var i = 0; i < lines.length; i++ ){
                    if(lines[i].payment_method.allow_for_wallet){
                        self.deletePaymentLine({ detail: { cid: lines[i].cid } });
                    }
                }
                let amount = client.remaining_wallet_amount >= order.get_due() ? order.get_due() : client.remaining_wallet_amount;
                if(client.remaining_wallet_amount == 0 || client.remaining_wallet_amount < 0){
                    return
                }
                const { confirmed, payload: redeemAmount } = await self.showPopup('AddMoneyToWalletPopup',{
                    customer: client.name || false,
                    rechargeAmount: amount,
                    flag : 'from_payment',
                    wallet_bal : client.remaining_wallet_amount,
                    title: self.env._t('Redeem Wallet'),
                });
                if (confirmed){
                    ...
                    if(payment_method){
                        ...
                        order.set_to_wallet(!order.is_to_wallet());
                        self.env.pos.load_new_partners();
                        self.state.remaining_wallet_amount -= order.selected_paymentline.amount;
                        self.render();
                        return
                    }
                }
            ...
            async validateOrder(isForceValidate){
                if(this.currentOrder.get_client()){
                    ...
                    && !this.currentOrder.get_client().add_change_wallet && this.currentOrder.get_type_for_wallet() != 'recharge'){
                    ...
                    }else if(this.currentOrder.get_client().add_change_wallet){
    ...
    Registries.Component.extend(PaymentScreen, WalletFrPaymentScreen);
