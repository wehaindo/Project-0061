...
class PosPaymentMethod(models.Model):
    ...
    allow_for_coupon = fields.Boolean(string="Coupon")

    @api.model
    def create(self, vals):
        res = super(PosPaymentMethod, self).create(vals)
        if vals.get('allow_for_coupon'):
            if vals.get('is_cash_count'):
                raise UserError(_('You can not use cash in case of coupon !!!'))
...
class PosConfig(models.Model):
    ...
    enable_coupon = fields.Boolean('Enable Coupon')
