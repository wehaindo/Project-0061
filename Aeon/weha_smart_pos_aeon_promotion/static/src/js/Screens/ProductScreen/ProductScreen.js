...
//                          this.currentOrder.apply_pos_promotion();
                        ...
                        }else if(promotion.promotion_type === 'buy_x_partial_quantity_get_special_price'){
                            for(const order_line of this.currentOrder.get_orderlines()){
                                if(order_line.get_promotion().id === promotion.id){
                                    var product = order_line.product                                    
                                    selectedLine.set_unit_price(val);                                         
                                    order_line.set_unit_price(order_line.product.get_price(this.env.pos.config.pricelist_id, val))
                                    order_line.set_promotion(false);
                                    order_line.set_price_source(order_line.product.get_price_source());
                                    order_line.set_prc_no(order_line.product.get_price_change_no());
                                    order_line.set_list_price(order_line.product.lst_price);                                
                                    order_line.set_combination_id('');
                                }
                            }
                            this.currentOrder.remove_orderline(selectedLine);
                        }else if(promotion.promotion_type === 'combination_product_fixed_price'){
                            for(const order_line of this.currentOrder.get_orderlines()){
                                if(order_line.get_promotion().id === promotion.id){
                                    var product = order_line.product                                    
                                    selectedLine.set_unit_price(val);                                         
