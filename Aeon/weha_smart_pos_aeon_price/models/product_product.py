...
class ProductTemplate(models.Model):
    ...
    def _prepare_product_json_by_branch(self, branch_id):
        _logger.info("Prepare Product JSON")
        product_template_couchdb_id, product_couchdb_id, product = super(ProductTemplate, self)._prepare_product_json(branch_id)
        #Update Store Price
        domain  = [
    ...
    def update_store_product_old(self, branchs):        
        for branch in branchs:        
            _logger.info('update_store_product')
            _logger.info(branch)        
            domain = [
                ('res_branch_id', '=', branch['store_id']),
                ('product_template_id', '=', branch['product_id'])
            ]
            product_template_price_id = self.env['product.template.price'].search(domain, limit=1)
            if not product_template_price_id:
                vals  = {
                    'res_branch_id': branch['store_id'],
                    'product_template_id': branch['product_id'],
                    'list_price': branch['list_price'],                        
                }
                self.env['product.template.price'].create(vals)
            else:
                product_template_price_id.list_price = branch['list_price']

    def update_store_product(self, vals):
        _logger.info('update_store_product')    
        _logger.info(vals)
        domain = [
            ('res_branch_id', '=', vals['res_branch_id']),
            ('product_template_id', '=', vals['product_template_id'])
        ]    
        _logger.info(domain)
        product_template_price_id = self.env['product.template.price'].search(domain, limit=1)
        _logger.info(product_template_price_id)
        if not product_template_price_id:
            _logger.info("Create Product Template Price")
            self.env['product.template.price'].create(vals)
        else:
            product_template_price_id.list_price = vals['list_price']
            
    def action_update_store_product_old(self):
        ...
        for row in self:
            for branch_id in row.branch_ids:
                ...
                _logger.info(store_pricelist_id)
                if store_pricelist_id:
                    ...
                    if product_pricelist_item_id:
                        product_pricelist_item_id.fixed_price = row.list_price
                    else:
                        vals = {
                            'pricelist_id': store_pricelist_id.id,
                            'product_tmpl_id':  row.id, #Product Template  
                            'fixed_price': row.list_price,                          
                        }
...
class ProductTemplatePrice(models.Model):
    ...
    def action_update_store_product(self):
        for row in self:
             row.product_template_id.action_sync_product_by_branch(row.res_branch_id.id)
    ...
    product_name = fields.Char('Product Name', related='product_template_id.name', store=True)
    list_price = fields.Float('Sale Price')
    member_price = fields.Float('Member Price')

    @api.model
    def create(self, vals):
        res  = super(ProductTemplatePrice, self).create(vals)
        return res

    def write(self, vals):
        super(ProductTemplatePrice, self).write(vals)    
