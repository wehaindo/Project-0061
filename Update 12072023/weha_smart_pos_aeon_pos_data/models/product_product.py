...
class ProductTemplate(models.Model):
    ...
    def _prepare_product_json(self, branch_id):
        ...
        sql_product_couchdb="""SELECT product_couchdb_id, branch_id, product_template_id FROM product_template_couchdb WHERE branch_id={} AND product_template_id={}""".format(branch_id.id, self.id)
        self.env.cr.execute(sql_product_couchdb)
        product_template_couchdb_id = self.env.cr.dictfetchall()

        for product_couchdb in product_template_couchdb_id:
            if not product_couchdb:
                product_couchdb_id = "product_" + str(uuid.uuid4())
                product['_id'] =  product_couchdb_id           
            else:
                product['_id'] =  product_couchdb['product_couchdb_id']
                product_couchdb_id = product_couchdb['product_couchdb_id']
    ...
    def generate_product_json(self):
        ...
        for branch_id in self.branch_ids:
            ...
            #         res = self.env['product.template.couchdb'].create(vals)
            #     else:
            #         sql_update="""UPDATE product_template_couchdb SET product_couchdb_sync='f', product_json='{}', product_couchdb_id='{}' 
            #                         WHERE branch_id={} AND product_template_id={}""".format(products_json, product_couchdb_id, branch_id.id, self.id)
