odoo.define('weha_smart_pos_aeon_mandiri_ecr.PaymentScreen', function(require) {
    "use strict";
    ...
    const MandiriEcrPaymentScreen = PaymentScreen => class extends PaymentScreen {
        setup() {   
            super.setup();
            useListener('check-mandiri-payment-qris', this._checkMandiriPaymentQris);
            onMounted(() => {
                const pendingPaymentLine = this.currentOrder.paymentlines.find(
                    paymentLine => paymentLine.payment_method.use_payment_terminal === 'mandiriecr' &&
        ...
        async _checkMandiriPaymentQris({ detail: line }){
            const payment_terminal = line.payment_method.payment_terminal;
            line.set_payment_status('waiting');
            const isPaymentSuccessful = await payment_terminal.check_mandiri_payment_qris(line.cid);
            if (isPaymentSuccessful) {
    ...
    Registries.Component.extend(PaymentScreen, MandiriEcrPaymentScreen);      
