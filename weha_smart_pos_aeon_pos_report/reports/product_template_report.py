...
class ProductTemplateReport(models.AbstractModel):
    ...
    def getData(self, store_id, line_id):
        if line_id:
            domain = [('branch_ids','in',[store_id.id], 'line_id', '=', line_id.id)]    
        else:
        ...
        for product_template_id in product_template_ids:            
            ...
            store_price = self.get_store_price(store_id, product_template_id)
            if store_price:
                vals.update({
                    'normal_price': store_price.list_price
                })
    ...
    def generate_xlsx_report(self, workbook, data, obj):
        # _logger.info(data)     
        lines = self.getData(obj.store_id. obj.line_id) 
        row = 0
