   ...
   const AeonPricePmsMemberPopup = (PmsMemberPopup) => 
        class extends PmsMemberPopup {
            ...
            clearCustomer(){
                ...
                super.clearCustomer();
                var order = self.env.pos.get_order();
                let default_pricelist_id = _.find(this.env.pos.pricelists, function (pricelist) {
                    return pricelist.id === self.env.pos.config.pricelist_id[0];});
                order.set_pricelist(default_pricelist_id)                 
            }
